...
const tensionColors: Record<TensionLevel, string> = {
  ...
  1: 'bg-yellow-500/30 border-yellow-500/40 text-yellow-400',
                ...
                <div className="flex gap-2">
                  {[0, 1, 2, 3].map((level) => (
                    <div key={level} className="flex items-center gap-1">
                      <div className={`w-2 h-2 rounded-full ${level === 0 ? 'bg-green-500' : level === 1 ? 'bg-yellow-500' : level === 2 ? 'bg-orange-500' : 'bg-red-500'}`} />
